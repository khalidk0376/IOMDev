({
    ...
    helperGetAllObjectList: function(component, event) {
        var action = component.get("c.getAllsObjectName"); //Calling Apex class controller 'getAllsObjectName' method
        action.setCallback(this, function(res) {
            var state = res.getState();
            if (state === "SUCCESS") {
                ...
                for (var i = 0; i < objlist.length; i++) {
                    opts.push({ label: objlist[i].apiLabelName, value: objlist[i].apiName });
                }
                component.set('v.lstObjectOptions', opts);
                var vQues = component.get("v.objCrteQues");
                if (vQues.lookup_Object_API_Name__c !== undefined && vQues.lookup_Object_API_Name__c !== "" && vQues.lookup_Object_API_Name__c.length !== 0) {
                    this.helperGetSelectedObjectFields(component, event, vQues.lookup_Object_API_Name__c);
                }
            } else {
                this.showToast(component,'error',res.getError()[0].message);
            }
    ...
    helperGetSelectedObjectFields: function(component, event, Object_API_Name) {
        var action = component.get("c.getAllsObjectFieldName");
        action.setParams({
        ...
        action.setCallback(this, function(res) {
            ...
            if (state === "SUCCESS") {
                ...
                component.set('v.lstObjectFieldOptions', opts);
            } else {
                this.showToast(component,'error',res.getError()[0].message);
            }
        });
