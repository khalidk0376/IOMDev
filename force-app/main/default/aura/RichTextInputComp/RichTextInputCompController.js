({
    ...
    saveQues: function(component, event, helper) {
        ...
        var charValue = component.get("v.objCrteQues.Max_Char_Length__c");
        ...
        if (charValue !== undefined && charValue !=="" && charValue.length !== 0) {
            var charLimit = parseInt(charValue, 10);
            if (charLimit === 0 || charLimit > 10000) {
                message = "Min to max range 1 to 10000";
                helper.showToast(component, 'error', message);
                component.set("v.isShowbutton", false);
                component.find('qustNameRich').focus();
                return false;
