...
export default class CommonTable extends LightningElement {
    ...
    setParentFieldValue(tbldatas){ 
        ...
        for(let i=0;i<datas.length;i++){
            ...
            if(datas[i].hasOwnProperty('OrderNumber')){
                datas[i].OrderNumberLink='/lightning/r/'+this.objectName+'/'+datas[i].Id+'/view';
            }
    ...
    setParentFieldColumn(columnObj,columnList,datas){
        ...
        for(let i=0;i<columnObj.length;i++){
            ...
            if(columnObj[i].fieldName!==undefined && 
                (columnObj[i].fieldName.toLowerCase().indexOf('name')>=0 || columnObj[i].fieldName.toLowerCase() === 'workordernumber'||columnObj[i].fieldName.toLowerCase() === 'ordernumber' || (columnObj[i].fieldName.toLowerCase().indexOf('value__c')>=0  && this.objectName !== 'Communication_Directory__c')|| columnObj[i].fieldName.toLowerCase().indexOf('levelvalue__c')>=0
    ...
    handleSorting(event){
        ...
        this.delayTimeout = setTimeout(()=>{
           ...
           this.sortByFieldName = sortField;
            ...
            if(sortFieldName.toLowerCase().indexOf('ordernumberlink')>=0 || sortFieldName.toLowerCase().indexOf('workordernumberlink')>=0 || sortFieldName.toLowerCase().indexOf('lineitemnumberlink')>=0){
                const n = sortFieldName.lastIndexOf('Link');
                sortFieldName = sortFieldName.slice(0, n) + sortFieldName.slice(n).replace('Link', '').trim();
            }
    ...
    handleRowSelection(event){
        ...
        this.dispatchEvent(new CustomEvent('rowselection',{detail:this.selectedRows}));
    ...
    get showActionList()
    {
        return (this.selectedRows && this.hideMassAction == false)?true:false;
    }
