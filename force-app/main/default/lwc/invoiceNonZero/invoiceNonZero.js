...
export default class InvoiceNonZero extends LightningElement {
  ...
  @api objectLabel = "Invoices";
  @api fields =
    "Name,blng__InvoiceStatus__c,blng__TotalAmount__c,blng__Order__r.SBQQ__Quote__r.Edition__r.Name,blng__InvoiceDate__c,blng__Account__r.Name,CreatedDate";
  @api fieldsLabel =
    "Invoice Number,Status,Total Amount (With Tax),Edition Name,Invoice Date,Account,Created Date";
  @api condition = "Name!=''";
  @track tempCondition = "";
  @api profile = "";
  ...
  //filter1
  @api filterField1 = "blng__Order__r.SBQQ__Quote__r.Edition__r.Name";
  ...
  connectedCallback() {
    ...
    // add currency code
    if((this.objectName.toLowerCase()==='blng__invoice__c') && this.fields.toLowerCase().indexOf('currencyisocode')<0){
    ...
    getSSCTeamMember()
      .then(result => {
        if (result) {
          ...
          this.condition ='((blng__InvoiceStatus__c=\'Draft\' OR blng__InvoiceStatus__c=\'Post In progress\') AND blng__Order__r.SBQQ__Quote__r.Edition__r.Part_of_Series__c IN (\''+ids.join('\',\'')+'\') AND blng__TotalAmount__c!=0.0)';
          this.showTable = true;
  ...
  // create custom event and fire to open mass update form can be used in future 
  openMassUpdateForm() {
