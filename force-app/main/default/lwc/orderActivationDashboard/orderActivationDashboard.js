...
export default class OrderActivationDashboard extends LightningElement {
  ...
  @api fields =
    "OrderNumber,Contract_Number__r.ContractNumber,Account.Name,Opportunity.Name,SBQQ__Quote__r.Name,Status,SBQQ__Quote__r.Edition__r.Name,Tax_Status__c,CreatedDate";
  @api fieldsLabel =
    "Order Number,Contract Number,Account Name,Opportunity,Quote,Status,Edition,Tax Status,Created Date";
    ...
    getSSCTeamMember()
      .then(result => {
        if (result) {
          ...
          this.condition = '(Status=\'Draft\' AND Tax_Status__c!=\'Error\' AND SBQQ__Quote__r.Edition__r.Part_of_Series__c IN (\'' + ids.join('\',\'') + '\'))';
          this.showTable = true;
  ...
  setParentFieldValue(tbldatas) {
    ...
    for (let i = 0; i < datas.length; i++) {
      //datas[i].Name = datas[i].OrderNumber;
      //build link
