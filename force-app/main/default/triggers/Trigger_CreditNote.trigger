 ...
 * *******************************************************************************************************
 * @description  Custom CreditNote trigger -: All trigger code can be added to this component
 * *******************************************************************************************************
 */
trigger Trigger_CreditNote on blng__CreditNote__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_CreditNote.Trig trig = new TriggerInterface_CreditNote.Trig();
    ...
    //SObject so = Schema.getGlobalDescribe().get('blng__CreditNote__c').newSObject();
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_CreditNote.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_CreditNote.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
