 ...
 * *******************************************************************************************************
 * @description  Custom CreditNoteLine trigger - All trigger code can be added to this component.
 * *******************************************************************************************************
 */
trigger Trigger_CreditNoteLine on blng__CreditNoteLine__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_CreditNoteLine.Trig trig = new TriggerInterface_CreditNoteLine.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('blng__CreditNoteLine__c');
    ...
    //SObject so = Schema.getGlobalDescribe().get('blng__CreditNoteLine__c').newSObject();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'blng__CreditNoteLine__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<blng__CreditNoteLine__c>();
                trig.newList = new List<blng__CreditNoteLine__c>();
                trig.oldMap = new Map<Id,blng__CreditNoteLine__c>();
                trig.newMap = new Map<Id,blng__CreditNoteLine__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_CreditNoteLine.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_CreditNoteLine.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_CreditNote (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_CreditNote (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10){
