 ...
 * *******************************************************************************************************
 * @description : Custom Invoice trigger - All trigger code can be added to this component.
 * *******************************************************************************************************
 */
trigger Trigger_Invoice on blng__Invoice__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
    TriggerInterface_Invoice.Trig trig = new TriggerInterface_Invoice.Trig();
    ...
    TriggerInterfaceHelper.initTriggerMapAndList('blng__Invoice__c');
    ...
    for (Trigger__c t : listTriggers) {
        ...
        System.debug(LoggingLevel.Debug, runTrigger);
        if (t.Object__c == 'blng__Invoice__c') {
            ...
            || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<blng__Invoice__c>();
                trig.newList = new List<blng__Invoice__c>();
                trig.oldMap = new Map<Id,blng__Invoice__c>();
                trig.newMap = new Map<Id,blng__Invoice__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_Invoice.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_Invoice.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug(LoggingLevel.Debug, '********** Trigger_Invoice (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug(LoggingLevel.Debug, '********** Trigger_Invoice (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10){
