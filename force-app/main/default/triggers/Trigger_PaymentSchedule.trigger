...
* Coverage:     92%
* *******************************************************************************************************
* @description : Trigger on Payment_Schedule__c that will run on before insert and it will cover TriggerInterface_PaymentSchedule, PaymentScheduleUpdate_Trig.
* *******************************************************************************************************
* History:  Modified By Yash Gupta [11/27/2019], Increase code coverage, Modified By Yash Gupta [1/28/2020], Increase code coverage.
*/

trigger Trigger_PaymentSchedule on Payment_Schedule__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    /**
    * Description: Calling TriggerInterface_PaymentSchedule class for Payment_Schedule__c object.
    */
    TriggerInterface_PaymentSchedule.Trig trig = new TriggerInterface_PaymentSchedule.Trig();
    ...
    /**
    * Description: Calling TriggerInterfaceHelper helper class for Payment_Schedule__c object.
    */
    TriggerInterfaceHelper.initTriggerMapAndList('Payment_Schedule__c');
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Payment_Schedule__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Payment_Schedule__c>();
                trig.newList = new List<Payment_Schedule__c>();
                trig.oldMap = new Map<Id,Payment_Schedule__c>();
                trig.newMap = new Map<Id,Payment_Schedule__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_PaymentSchedule.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_PaymentSchedule.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
